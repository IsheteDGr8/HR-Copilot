...
function ClusterPageContent({ clusterId: propClusterId }: { clusterId?: string }) {
  ...
  const targetClusterId = propClusterId || nav.selectedClusterId || intakeClusters[0]!.id;
  const cluster = intakeClusters.find((c) => c.id === targetClusterId) || intakeClusters[0]!;
  ...
  return (
    <PageContainer>
      ...
      <div className="mt-3 flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="panel flex items-center gap-5 px-4 py-3 rounded-lg border border-border bg-white">
          <div>
      ...
      {cluster.discovered && (
        <div className="panel mt-4 flex flex-wrap items-center gap-3 border-warning/40 bg-amber-500/5 px-4 py-3 rounded-lg border">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_400px]">
        <section className="panel overflow-hidden rounded-lg border border-border bg-white">
            ...
            .map((i) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex w-full items-start gap-3 border-b border-border px-4 py-3 text-left transition-colors last:border-0 hover:bg-sidebar-accent/60",
...
function TriagePanel({ item, cluster }: { item: IntakeItem; cluster: IntakeCluster }) {
  ...
  return (
    <aside className="panel h-fit lg:sticky lg:top-20 rounded-lg border border-border bg-white">
...
export default function IntakeClusterDetail({ clusterId }: { clusterId?: string }) {
  return <ClusterPageContent clusterId={clusterId} />
}
