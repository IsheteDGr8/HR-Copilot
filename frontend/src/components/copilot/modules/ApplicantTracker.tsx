...
export function ApplicantTracker({ data }: Props) {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <header className="rounded-xl border border-white/[0.08] bg-gradient-to-br from-white/[0.04] to-transparent p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <h2 className="mt-1 truncate text-[16px] font-semibold text-neutral-50">
              {jobRole || "Open requisition"}
            </h2>
            <p className="mt-1 text-[12px] text-neutral-400">
              Req <span className="font-mono text-neutral-300">{requisitionId}</span>
          ...
          <button
            ...
            className="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-md border border-white/10 px-2.5 text-[12px] text-neutral-300 transition hover:bg-white/5"
          >
            {loading ? (
      ...
      {rows.length === 0 ? (
        <p className="rounded-xl border border-dashed border-white/10 px-4 py-10 text-center text-[13px] text-neutral-500">
          ...
          {rows.map((a) => {
            ...
            return (
              ...
              >
                <div className="flex flex-col gap-4 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex min-w-0 items-start gap-3">
                      <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white/[0.06]">
                        <UserRound className="h-5 w-5 text-neutral-200" />
                      </span>
                      <div className="min-w-0">
                        <h3 className="truncate text-[15px] font-semibold text-neutral-50">
                          {a.name || "Candidate"}
                        </h3>
                        <p className="mt-0.5 flex items-center gap-1.5 truncate text-[12.5px] text-neutral-400">
                    ...
                    <div
                      className={cn(
                        "flex h-[4.25rem] w-[4.25rem] shrink-0 flex-col items-center justify-center rounded-full border bg-black/30",
                        tone.ring,
                      )}
                    >
                      ...
                      <span className="mt-1 text-[9px] font-medium uppercase tracking-wide text-neutral-500">
                        match
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between text-[11px]">
                      ...
                      <span className="tabular-nums text-neutral-500">{score}/100</span>
                    </div>
                    <Progress
                      value={score}
                  ...
                  <p className="text-[13px] leading-relaxed text-neutral-300">{summary}</p>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {skills.map((s) => (
                            <Badge
                              key={`s-${id}-${s}`}
                              className="border-emerald-500/25 bg-emerald-500/15 text-emerald-100 hover:bg-emerald-500/20"
                            >
                              {s}
                            </Badge>
                          ))}
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {gaps.map((s) => (
                            <Badge
                              key={`g-${id}-${s}`}
                              variant="outline"
                              className="border-rose-500/25 bg-rose-500/10 text-rose-200/90"
                            >
                              {s}
                            </Badge>
                          ))}
                ...
                <div className="mt-auto flex flex-wrap gap-2 border-t border-white/[0.07] bg-black/25 px-4 py-3">
                  <button
                    type="button"
                    disabled={busy}
                    onClick={() => void patch(a, { status: "Shortlisted" }, "Candidate shortlisted")}
                    className="inline-flex h-9 flex-1 items-center justify-center gap-1.5 rounded-md border border-white/15 bg-white/[0.08] px-3 text-[12.5px] font-medium text-neutral-50 transition hover:bg-white/[0.14] disabled:opacity-50"
                  >
                    {busy ? (
                      <Loader2 className="h-3.5 w-3.5 animate-spin" />
                    ) : (
                  ...
                  <button
                    ...
                    className="inline-flex h-9 flex-1 items-center justify-center gap-1.5 rounded-md border border-sky-500/30 bg-sky-500/10 px-3 text-[12.5px] font-medium text-sky-100 transition hover:bg-sky-500/20 disabled:opacity-50"
                  >
                    {busy ? (
                      <Loader2 className="h-3.5 w-3.5 animate-spin" />
                    ) : (
                  ...
                  <button
                    ...
                    className="inline-flex h-9 items-center justify-center gap-1.5 rounded-md border border-white/10 bg-transparent px-3 text-[12.5px] font-medium text-neutral-300 transition hover:bg-white/5 disabled:opacity-50"
                  >
                    {busy ? (
                      <Loader2 className="h-3.5 w-3.5 animate-spin" />
                    ) : (
